...
def isinvalidcaptchascreen(driver):
    try:
        ...
        #logandshow(invalidcaptcha.text)
...
def isentervalidcaptchapopup(driver):
    try:
        ...
        if "Enter" in entervcaptcha.text:
...
for cnr in cnrlist:
    ...
    logandshow("       ")
    logandshow("       ")
    logandshow(str(donecount) + " of " + str(cnrcount)+ "  CNR = "+cnr+"   ############################################")
    logandshow("       ")
    #driver.get(cnrlink)
    time.sleep(1)
    while retval!="OK":
...
df = pandas.DataFrame(AllData,columns=['CNR', 'CASE TYPE', 'FILING NUMBER', 'FILING DATE', 'REG NUMBER', 'REG DATE',
                                       'FIRST HEARING', 'NEXT HEARING/DECISIONDATE', 'STAGE','COURT', 'PETIONER','NATUREOFDISPOSAL'])
